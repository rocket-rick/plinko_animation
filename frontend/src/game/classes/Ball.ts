...
export class Ball {
  ...
  constructor(x: number, y: number, radius: number, color: string, ctx: CanvasRenderingContext2D, obstacles: Obstacle[], sinks: Sink[], onFinish: (index: number) => void) {
    ...
    this.path = [true, true, true, true, true, true, true, true, true, false, false, false]
  ...
  update(deltaTime: number) {
    ...
    this.obstacles.forEach(obstacle => {
      ...
      if (dist < pad(this.radius + obstacle.radius)) {
        ...
        const rotateAngle = this.current_index++ ? this.path[this.current_index] ? 30 : - 30 : 0;
        console.log(this.current_index, rotateAngle)
        const angle = Math.atan2(this.y - obstacle.y, this.x - obstacle.x) + rotateAngle;
        // Reflect velocity
