...
export class Ball {
  ...
  constructor(x: number, y: number, radius: number, color: string, ctx: CanvasRenderingContext2D, obstacles: Obstacle[], sinks: Sink[], target: Point, onFinish: (index: number) => void) {
    this.x = x;
    this.y = y;
    this.radius = radius;
    this.color = color;
  ...
  update(deltaTime: number) {
    ...
    if (distance > pad(this.radius)) {
      ...
      const steeringForce = 100; // Adjust if needed
      this.vx += normalizedDirectionX * steeringForce * 1;
      this.vy += normalizedDirectionY * steeringForce * 1;
    } else {
      // Stop the ball if it reaches near the target
      this.vx = 0;
      this.vy = 0;
    }
